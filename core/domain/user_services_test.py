...
class UserServicesUnitTests(test_utils.GenericTestBase):
    ...
    def test_get_system_user_returns_system_user_action_info(self) -> None:
        ...
        expected_actions = set(
            [
                ...
                'RATE_ANY_PUBLIC_EXPLORATION',
                'DELETE_ANY_PUBLIC_ACTIVITY',
                'MANAGE_ACCOUNT',
                'MODIFY_CORE_ROLES_FOR_OWNED_ACTIVITY',
                'CREATE_EXPLORATION',
                'UNPUBLISH_ANY_PUBLIC_ACTIVITY',
                'CHANGE_TOPIC_STATUS',
                'SEND_MODERATOR_EMAILS',
                'FLAG_EXPLORATION',
                'ACCESS_CREATOR_DASHBOARD',
                'EDIT_ANY_TOPIC',
                'ACCEPT_ANY_SUGGESTION',
                'PUBLISH_OWNED_ACTIVITY',
                ...
                'SUGGEST_CHANGES',
                'DELETE_OWNED_PRIVATE_ACTIVITY',
                'EDIT_OWNED_ACTIVITY',
                'EDIT_SKILL_DESCRIPTION',
                'DELETE_ANY_ACTIVITY',
                'SUBSCRIBE_TO_USERS',
                'PLAY_ANY_PRIVATE_ACTIVITY',
                'MANAGE_QUESTION_SKILL_STATUS',
                'MODIFY_CORE_ROLES_FOR_ANY_ACTIVITY',
                'ACCESS_TOPICS_AND_SKILLS_DASHBOARD',
                'EDIT_SKILL',
                'DELETE_ANY_QUESTION',
                'EDIT_ANY_STORY',
                'PUBLISH_ANY_ACTIVITY',
                'EDIT_ANY_QUESTION',
                'CREATE_NEW_SKILL',
                'CHANGE_STORY_STATUS',
                'CAN_MANAGE_VOICE_ARTIST',
                'ACCESS_LEARNER_GROUPS',
    ...
    def test_update_users_data_for_multiple_users_works_correctly(self) -> None:
        ...
        profile_user_ids = [
            user_settings_list[0].user_id,
            user_settings_list[1].user_id,
        ]
...
"""
    ...
    def test_sync_logged_in_learner_checkpoint_progress_with_current_exp_version(  # pylint: disable=line-too-long
        ...
        self.login(self.VIEWER_EMAIL)
        exp_user_data = user_services.sync_logged_in_learner_checkpoint_progress_with_current_exp_version(  # pylint: disable=line-too-long
            self.viewer_id, self.EXP_ID
        )
        self.assertIsNone(exp_user_data)
...
class UserDashboardStatsTests(test_utils.GenericTestBase):
    ...
    def test_get_user_dashboard_stats(self) -> None:
        ...
        self.assertEqual(
            ...
            {'total_plays': 1, 'num_ratings': 0, 'average_ratings': None},
        )
...
class LastLoginIntegrationTests(test_utils.GenericTestBase):
    ...
    def test_legacy_user(self) -> None:
        ...
        self.assertLess(last_logged_in, new_last_logged_in)
        self.logout()
...
class TranslationCoordinatorRightsTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.set_translation_coordinators(
            [user_services.get_username(self.user_id_translationcoordinator)],
            'en',
        )
