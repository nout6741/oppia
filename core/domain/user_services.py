...
LABEL_FOR_DELETED_USER: Final = '[User deleted]'
...
def get_all_user_groups() -> List[user_domain.UserGroup]:
    ...
    for user_group_model in user_group_models:
        ...
        for user_id, user_settings in zip(user_ids, user_settings_list):
            ...
            assert (
                user_settings.username is not None
...
def create_new_user_group(
    ...
    user_group = user_domain.UserGroup(user_group_id, name, member_usernames)
    user_group.validate()
...
def get_human_readable_user_ids(
    ...
    Args:
        ...
        strict: bool. Whether to fail noisily if no user with the given
            id exists in the datastore. Defaults to True.
        include_deleted: bool. Whether to include users that are
...
def get_translation_rights_from_model(
    translation_coordinator_model: suggestion_models.TranslationCoordinatorsModel,
